...
export function useArticleSeo(opts: ArticleSeoOpts) {
  useEffect(() => {
    ...
    upsertMeta('name', 'author', 'Angel Lambertt')
    ...
    upsertMeta('property', 'og:site_name', 'alambertt.dev')
    ...
    for (const { hreflang, href } of [
      ...
      { hreflang: 'x-default', href: `https://www.alambertt.dev/${defaultSlug}` },
    ]) {
...
export function useHomeSeo({ lang, title, description }: { lang: string; title: string; description: string }) {
  useEffect(() => {
    ...
    const canonical = lang === 'en' ? 'https://www.alambertt.dev/en' : 'https://www.alambertt.dev/'
